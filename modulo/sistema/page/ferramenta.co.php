...
require_once "modulo/sistema/class/FerramentaBO.php";

Class Ferramenta extends ControllerMaster
{
	//Métodos
	function listar()
	{
		$this->DataGrid = new DataGrid($this->BO, "fer_id");
		$this->DataGrid->set_btInserir("sistema/ferramenta/inserir");
		$this->DataGrid->set_field("fer_id", "FER_ID", "", "");
		$this->DataGrid->set_field("fer_nome", "FER_NOME", "", "");
		$this->DataGrid->set_field("fer_page", "FER_PAGE", "", "");
		$this->DataGrid->set_field("mod_id", "MOD_ID", "", "");
		$this->DataGrid->set_tool("Exibir", "sistema/ferramenta", "glyphicon glyphicon-file", "Exibir");
		if($this->Seguranca->verificaAcesso("excluir", $this->page))
		{
			$this->DataGrid->set_tool("Excluir", "sistema/ferramenta", "glyphicon glyphicon-trash", "Excluir", "Excluir");
		}
	...
	function salvar()
	{
		...
		$this->VO->set_fer_nome($_POST["fer_nome"]);
		$this->VO->set_fer_page($_POST["fer_page"]);
		...
		if((int)$this->VO->get_fer_id())
		{
			...
			header("Location:".$this->redirect."/editar/".$this->VO->get_fer_id());
			exit;
		}
		else
		{
			...
			$this->VO->set_fer_id($this->BO->ultimoId());
...
$Ferramenta = new Ferramenta("ferramenta", "sistema", "Ferramenta");
?>